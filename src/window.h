...
#define draw_texture(t, p, s, uvp, uvs) window_renderer_texture(&(window_renderer), (t), (p), (s), (uvp), (uvs))
...
static const char *window_renderer_fragment_shader_source=
  ...
  "        fragColor = out_color;\n"
  "    } else if(out_uv.x > 1 && out_uv.y > 1) {\n"
  "        vec4 color = texture(texture0, vec2(out_uv.x - 1, 1 - out_uv.y - 1));\n"
...
WINDOW_DEF void window_renderer_texture(Window_Renderer *r, unsigned int texture, Window_Renderer_Vec2f p, Window_Renderer_Vec2f s, Window_Renderer_Vec2f uvp, Window_Renderer_Vec2f uvs)
{
  ...
  GLint uniformLocation = glGetUniformLocation(r->program, "texture0");
  glUniform1i(uniformLocation, (int) texture);
